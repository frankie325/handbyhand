...
# torchvision 0.28 renamed the v2 image/box helpers and moved datapoints to
# tv_tensors. Keep the names used by the upstream YAML files as compatibility
# aliases so the official configs remain usable with the project's environment.
@register
...
@register
class ConvertDtype(T.ConvertImageDtype):
    pass
    ...
    def _transform(self, inpt: Any, params: Dict[str, Any]) -> Any:  
        if self.out_fmt:
            ...
            inpt = tv_tensors.BoundingBoxes(
                inpt,
                format=self.data_fmt[self.out_fmt],
