...
from dinov2.distributed import is_main_process, get_global_size, is_enabled
...
def get_args_parser(add_help: bool = True):
    ...
    parser.add_argument(
        "--config-file",
        default="dinov2/configs/train/vitl14.yaml",
        metavar="FILE",
...
def _identity_target(_):
    """自监督训练不使用标签，返回空元组。模块级函数可被 pickle（lambda 不行）。"""
    return ()
...
def do_train(cfg, model, resume=False):
    model.train()
    # 仅在启用分布式(FSDP 的 MixedPrecision 会把参数也转 fp16)时用 fp16 输入；
    ...
    # checkpointer
    # 未启用分布式（单机单卡、跳过 FSDP）时，模型内没有 FSDP 子模块，
    # 不能用 FSDPCheckpointer（它依赖 FSDP.state_dict_type），改用普通 Checkpointer。
    checkpointer_cls = FSDPCheckpointer if is_enabled() else Checkpointer
    ...
    )  # OFFICIAL_EPOCH_LENGTH表示一个epoch最大的迭代次数
    ...
    n_tokens = (img_size // patch_size) ** 2  # 一张特征图patch的数量
    ...
    """
    # 数据集由配置文件中的 train.dataset_path 指定（不再硬编码），
    # 格式为 "ImageFolder:root=<路径>" 或 "ImageNet:split=TRAIN" 等。
    # 路径相对于运行时的 cwd（即 dinov2 包的父目录）。上传服务器后
    # 只需修改对应 yaml 里的 dataset_path 即可切换数据集。
    dataset = make_dataset(
        dataset_str=cfg.train.dataset_path,
        transform=data_transform,
        target_transform=_identity_target,
    )
...
def main(args):
    ...
    # 分布式环境已在 setup() -> default_setup() 中通过 distributed.enable(overwrite=True) 启用
    # （多卡时初始化 NCCL 进程组，单卡/Windows 则跳过，走普通 Checkpointer 与跳过 FSDP）。
    # 注意：此处不能重复调用 enable()，否则多卡下会触发
